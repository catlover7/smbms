...
$(function(){
	...
	proCode.on("blur",function(){
		...
		//显示友情提示
		validateTip(proCode.next(),{"color":"#666666"},"* 请输入企业编码",false);
	}).focus();
	
	proName.on("focus",function(){
		validateTip(proName.next(),{"color":"#666666"},"* 请输入企业名称",false);
	}).on("blur",function(){
		if(proName.val() != null && proName.val() != ""){
			validateTip(proName.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(proName.next(),{"color":"red"},imgNo+" 企业名称不能为空，请重新输入",false);
		}
